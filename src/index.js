...
function checkGuess(playerGuess, targetNumber, guessCount) {
  ...
  if (playerGuess === targetNumber) {
      return [`🎉 You've guessed it in ${guessCount} tries! The number was ${targetNumber}. Game reset.`, true];
  } else if (playerGuess > targetNumber) {
      return [`Too high! Your guess: ${playerGuess}, Attempts: ${guessCount}. Try again!`, false];
  } else {
      return [`Too low! Your guess: ${playerGuess}, Attempts: ${guessCount}. Try again!`, false];
  }
}
...
async function handle_advance(data) {
  ...
  try {
    ...
    const [resultMessage, isCorrect] = checkGuess(playerGuess, targetNumber, guessCount);

    if (isCorrect || guessCount >= MAX_GUESSES) {
        if (!isCorrect) {
            log.info(`Out of guesses! The correct number was ${targetNumber}. Game reset.`);
        }
    ...
    const response = await axios.post(`${rollupUrl}/notice`, { payload: utf8ToHex(resultMessage) });
    log.info(`Notice status: ${response.status}, body: ${response.data}`);
} catch (error) {
    requestStatus = 'reject';
    const errorMessage = `Error processing request ${data}\n${error.stack}`;
    log.error(errorMessage);
    const response = await axios.post(`${rollupUrl}/report`, { payload: utf8ToHex(errorMessage) });
